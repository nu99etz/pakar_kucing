...
class GejalaController extends MainController
{
    ...
    public function ajax()
    {
        ...
        foreach($data as $value) {
            ...
            if($value['penjelasan_gejala'] == '') {
                $row[] = '-';
            } else {
                $row[] = '<button type="button" name="lihat" action="' . base_url().'gejala/penjelasan_gejala/'.$value['id'] . '" class="btn-lihat btn btn-flat btn-primary btn-sm"><i class = "fa fa-eye"></i> Lihat Penjelasan Gejala</button> ';
            }
            $button ='<button type="button" name="update" action="' . base_url().'gejala/edit/'.$value['id'] . '" class="btn-edit btn btn-flat btn-warning btn-sm"><i class = "fa fa-edit"></i></button> ';
            $button .= '<button type="button" name="delete" action="' . base_url().'gejala/destroy/'.$value['id'] . '" class="btn-delete btn btn-flat btn-danger btn-sm"><i class = "fa fa-trash"></i></button> ';
            $row[] = $button;
            $no++;
            $record[] = $row;
        }
